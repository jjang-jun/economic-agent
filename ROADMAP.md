...
2. 실제 첫 거래부터 `trade:record`로 기록하고 `portfolio:sync-secret`으로 Actions 포트폴리오를 동기화한다.
3. 성과 리뷰를 고도화한다: 섹터, 신호, 리스크팩터, 프롬프트 버전별 승률과 초과수익률을 비교한다.
4. 대시보드를 고도화한다: 기간 필터, 추천 상세, 실제 거래 상세, 리스크 차단 사유를 볼 수 있게 한다.
5. 브리핑/리포트 입력을 로컬 JSON fallback보다 Supabase 조회 중심으로 전환한다.
