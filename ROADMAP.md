...
## 현재 가장 중요한 다음 작업
1. 추천 실패 원인 사후 분류와 섹터별/리스크팩터별 승률 리포트를 자동화한다.
2. Telegram 승인 흐름을 실제 앱에서 주기적으로 점검하는 smoke script를 추가한다.
3. 가격 provider 실패 로그를 별도 테이블로 남겨 성공률뿐 아니라 실제 호출 실패율까지 계산한다.
4. 대시보드를 정적 HTML 생성 방식에서 Agent Server의 인증된 `/dashboard` 화면으로 승격할지 검토한다.
5. 다이제스트 전송 성공 후 `alert_events` 상태 전환 결과를 운영 리포트에서 별도 추적한다.
