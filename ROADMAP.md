...
- [ ] 국가법령정보·국회 의안정보를 연결해 정부안 → 국회 제출 → 통과 → 공포 → 시행을 동일 정책 사건으로 추적
  ...
  - 국가법령정보 공동활용의 공포일·공포번호·시행일 수집과 기본 법률명 사건 연결 골격 구현 완료. `OC` 승인·Secret 등록 및 로컬 실데이터 8개 법률 smoke 완료. GitHub-hosted runner의 `law.go.kr:443` 연결 제한 때문에 PC worker가 해당 소스를 담당
