...
KRX Open API는 키 발급 후에도 `sto/stk_bydd_trd`, `sto/ksq_bydd_trd` 같은 서비스별 권한이 열려 있어야 한다. Cloud Run/GitHub Actions에 `KRX_OPENAPI_KEY`를 넣은 뒤 `401 Unauthorized API Call`이 나오면 KRX 포털의 API 이용신청/승인 상태를 먼저 확인한다.
...
`SUPABASE_SERVICE_ROLE_KEY`는 서버 전용 secret이다. KIS 접근토큰을 `api_token_cache`에 저장해 Cloud Run, GitHub Actions, 로컬 실행이 같은 24시간 토큰을 재사용할 때 필요하다. 브라우저나 클라이언트 코드에는 절대 노출하지 않는다.
...
GitHub Actions의 `deploy-freshness.yml`은 평일 09:20 KST에 `/version`의 `commitSha`와 현재 `main` 커밋을 비교한다. Cloud Build 트리거는 저장소의 `cloudbuild.yaml`을 사용해야 하며, 이 파일은 이미지 태그·revision label과 `COMMIT_SHA` 환경변수를 같은 커밋으로 갱신한다. 서버 URL은 repository variable/secret `AGENT_SERVER_URL`로 기본값을 덮어쓸 수 있다.
