...
- 2026년에 폐기되는 구 국회 의안 API 대신 열린국회정보 `법률안 심사 및 처리(의안검색)` API를 선택형 정책 소스로 추가했다. 세금·부동산·자본시장·연금 관련 8개 법률명의 전체 페이지를 순회하고 의안번호, 제안자, 소관위/법사위 처리와 본회의 결과를 정규화한다. 실키로 중복 제거한 1,410건을 조회했으며, 의원 제출 초안 1,374건이 알림을 덮지 않도록 정부안은 제출부터, 의원·위원장 안은 본회의 통과 이후만 편입하는 중요도 게이트를 적용해 운영 대상 33건으로 줄였다. 로컬 키와 GitHub Secret 등록 및 통합 dry-run(공식 소스 6/6, 총 143건 수집)을 완료했고, 공포·시행 교차검증은 신청한 법제처 `OC` 승인 뒤 연결한다.
- 법제처 국가법령정보 공동활용 `lawSearch.do?target=law` 선택형 소스를 추가했다. `LAW_OPEN_DATA_OC`가 설정되면 추적 법률의 공포일·공포번호·시행일·제개정 구분을 정규화하고, 미래 시행일은 `공포`, 도래한 시행일은 `시행`으로 구분한다. 국회 개정안과 현행 법령은 별도 출처 ID를 유지하면서 기본 법률명 기반 동일 사건 키로 연결한다. 현재는 승인 대기라 기본 비활성화 상태이며 mock 계약 테스트만 수행한다.
