...
`action:report`의 신규/관찰 후보는 추천 당시 기준가와 최신 현재가를 함께 표시합니다. 현재가는 리포트 생성 시점에 다시 조회하되, 조회 실패 시에도 추천 당시 기준가와 손절 기준으로 리포트 생성을 계속합니다.
...
| `freedom-report.yml` | 평일 16:20 KST | 경제적 자유 목표 달성률과 목표 속도 점검 |
...
| `collector-ops-report.yml` | 평일 23:50 KST | 수집기 운영 상태 점검 |
| `price-provider-ops-report.yml` | 평일 23:55 KST | 가격 provider 품질 점검 |
...
`src/config/freedom.js`에서 월 생활비, 월 투자 가능 금액, 목표 순자산, 목표 인출률, 목표일, 기대 연수익률을 수정합니다.
